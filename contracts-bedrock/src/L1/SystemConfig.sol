...
contract SystemConfig is OwnableUpgradeable, ISemver, IGasToken {
    ...
    function setGasPayingToken(address _token, uint8 _decimals, string memory _name, string memory _symbol) external onlyOwner {
        ...
        (bool success, ) = portal.call(abi.encodeWithSignature("setGasPayingToken(address,uint8,bytes32,bytes32)", _token, _decimals, name, symbol));
